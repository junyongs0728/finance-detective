...
- [재무탐정 공개 웹사이트](https://finance-detective.junyongs0728.chatgpt.site): 기업과 질문을 선택해 실제 계산·공시 설명·답변 보류의 기록을 확인하는 정적 화면. `showcase/`에 같은 소스를 보관한다.
- `showcase/project-brief.pdf`: 원문 링크, 기술 선택, 실패 개선, 평가 범위가 포함된 2페이지 소개 자료.
- `showcase/evaluation.json`: 개발 평가 기록에서 실행 식별자를 제외한 소개용 사본.
- 소스 저장소와 실행 예시 웹사이트는 공개되어 있다. 이력서에는 웹사이트와 GitHub 링크를 함께 넣었다. 웹사이트는 저장된 실제 결과를 보여주며, 자유 질문이 가능한 실시간 AI 운영 서비스로 표현하지 않는다.
