...
기준: v0.3 / 2026-09-21. 코드로 구현한 범위와 확장 과제를 구분한다. 이 문서를 먼저 읽고 [Agent 구현 가이드](agent-graph-guide.md), [RAG 구현 가이드](rag-build-guide.md), [비용 제어 가이드](postgres-cost-guide.md) 순으로 보면 된다.
...
백엔드는 모듈을 분리한 **단일 FastAPI 애플리케이션**이다. 마이크로서비스를 운영하고 있다고 표현하지 않는다. PostgreSQL과 Neo4j만 Docker Compose로 실행하고, Python 서버와 Vite 개발 서버는 로컬 프로세스로 실행한다. 프런트 빌드 후에는 FastAPI가 React 정적 파일도 제공한다.
...
현재 1~3단계 경로만으로는 PostgreSQL JOIN으로도 구현할 수 있다. Neo4j는 채용 목표와 관계 모델링·탐색 학습을 고려한 의도적 선택이다. 장점과 함께 DB 두 개의 운영·복구·동기화 비용도 생긴다. 양쪽 DB를 한 번에 커밋하는 분산 트랜잭션이나 자동 outbox worker는 아직 없다.
...
8. `tests/`와 `evals/`: 테스트 통과와 실제 모델 정확도를 구분해 설명할 수 있는가?

라우트·컴포넌트·환경 설정·DB 연결은 다른 서비스에도 재사용할 수 있는 골격 코드다. 실제 프로젝트의 설계 기여는 공시 범위, 수치 관측값 정체성, 계산 도구 계약, 검색 전략, 검증·유보, 비용 예약, 실행 이력과 실패 개선에 있다. 라이브러리를 설치했다는 사실보다 이 경계를 코드와 사례로 설명하는 것이 중요하다.
