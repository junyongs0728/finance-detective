...
2026-09-16 구현. 금융 해석 수업 대신 실제 AI 서비스의 저장·검색·비용 경로를 설명한다.
...
현재 비용·캐시·pgvector·이용 코드 경로는 실행되는 구현이다. Agent 도구 선택, Ontology/Property Graph, 공개 회원가입/결제/SSO는 구현되지 않았다.
...
BM25와 RRF 결합은 아직 Python에 있다. 따라서 모든 검색 연산을 DB로 옮겼다고 설명하면 안 된다. 관계형 테이블의 FK나 HNSW 내부 그래프도 지원 공고의 **도메인 Ontology/Property Graph 구현**과는 다르다.
...
실제 이전: 문서 5건, 문단 1,804건, 임베딩 1,804건, 실행 기록 26건. 이전 파서 버전의 문서도 기록으로 보존된다. 현재 검색은 해당 공시·파서·모델 버전의 문서 ID만 조회한다.
...
최종 테스트 56개와 프런트엔드 빌드가 통과했다. 삼성전자 DART 공시도 실제 질문에서 근거 2개를 포함해 응답했고 생성·검토·질문 임베딩 추정 비용은 $0.015548이었다.
