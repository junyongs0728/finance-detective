...
모델은 연산과 관측값을 선택하지만, 조회하지 않은 숫자나 호환되지 않는 기간·통화의 계산은 서버가 거부한다. 이 경계는 [test_agent_graph.py](../tests/test_agent_graph.py)의 입력 검증·계산·실행 제한 검사로 확인한다.

## 8. 완성 범위와 다음 운영 과제

현재 요청 단위 Agent와 근거 추적은 동작한다. 멀티턴 장기 기억, durable checkpoint, 장애 후 Agent 재개, 다기업 비교, 범용 GraphRAG, MCP 도구 서버는 범위 밖이다.
