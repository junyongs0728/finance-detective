...
2026-09-16 구현. 이번 단계는 고정된 RAG 워크플로다. 도구 선택 Agent와 지식 그래프는 아직 아니다.

## 사용해보기

기업 검색에서 삼성전자 또는 애플을 선택하고 다음과 같이 질문한다.
...
위 모듈은 모두 실행되는 코드다. `agents/`는 여전히 확장 자리이며, LangGraph·Tool Calling·Ontology·Graph DB는 이 단계에 구현하지 않았다.

## SQL이 맡는 역할

`data/processed/rag.sqlite3`에 다음 네 종류의 데이터를 저장한다.

- `documents`: 회사 ID, 공시 번호, 출처, 기준 기간, 파서 버전, 임베딩 모델·차원, 상태, 원문 해시.
- `chunks`: 공시 외래키, 문단 순번, 절 제목, 원문 텍스트, 텍스트 해시.
- `embeddings`: 문단 외래키와 512차원 벡터(float32). 외래키가 없는 벡터는 저장할 수 없다.
...
작은 로컬 프로젝트라 SQLite와 Python의 벡터 내적으로 시작했다. 검색 범위를 한 공시로 좁히면 현재 규모에서는 구조를 읽고 디버깅하기 쉽다. 별도 벡터 DB와 근사 최근접 검색은 아직 없다. 여러 기업을 한 번에 검색하거나 동시 사용자/문서 수가 커질 때 병목을 측정하고 변경한다.
...
Structured Outputs는 JSON 형식 준수에 도움이 된다. 진실성을 보장하지 않으므로 Python에서 상태/주장/출처 ID를 검증한 뒤, 다른 호출로 근거 검토를 수행한다. 검토가 실패하면 후보 주장을 사용자에게 사실처럼 보여주지 않는다. 거절된 후보와 검토 내용은 로컬 실행 기록에 남긴다.

기본 생성 모델은 `gpt-4.1-mini`, 검토 모델은 `gpt-4.1`이다. 환경변수로 바꿀 수 있고 실제 사용한 모델 스냅샷은 실행 기록에 남긴다. 응답 저장 옵션은 `store=False`, 자동 SDK 재시도는 0, 호출 시간 제한은 45초다. 답변당 질의 임베딩 1회, 생성 1회, 답변 가능한 경우 근거 검토 1회가 발생한다. 첫 문서 준비의 임베딩은 별도다. 로컬 `runs`에는 질문이 저장되므로 DB 파일은 Git에서 제외한다.
...
현재 파서 버전의 실제 준비 결과는 삼성전자 95개, 애플 213개, 쿠팡 482개 서술 문단이다. 이전 파서의 표 포함 색인은 별도 버전으로 남으며 새 검색에는 섞이지 않는다. 표·표 내부 문단·첨부·이미지/OCR는 이번 범위 밖이다. 원문 크기 40MB, 최대 1,600개 문단, 상위 검색 6개 등의 한도를 둔다.
...
다음으로 할 일은 질문-정답 근거 쌍을 늘려 BM25 단독/임베딩 단독/하이브리드를 비교하고, 잘못된 답변 유보와 과도한 유보를 각각 측정하는 것이다. 그 뒤 기존 도구의 입력·출력을 명시해 Agent의 도구 선택으로 확장한다.
